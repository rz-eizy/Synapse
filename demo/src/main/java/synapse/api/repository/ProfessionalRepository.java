...
public interface ProfessionalRepository extends JpaRepository<Professional, UUID> {    
    @Query("SELECT new synapse.api.dto.ProfessionalProfileDTO(" +
           ...
           "COALESCE(AVG(r.stars), 0.0), COUNT(r), p.personalContact) " +
           "FROM Professional p " +
           "JOIN p.user u " +
           "LEFT JOIN p.ratings r " +
           "WHERE p.id = :professionalId " +
           "GROUP BY p.id, p.professionName, p.institutions, p.yearsExperience, p.city, p.businessHours, p.modality, p.professionalDescription, p.costWork, u.username, u.profilePictureUrl, p.personalContact")
